...
class EKF:
    ...
    def g(self, 
        ...
        '''
        print('mu',self.mu)
        # mu is commanded [phi,y_dot,y] where in this case y_dot and y are 1.0
        current_phi, current_y_dot, current_y = self.mu

        # TODO: return the g matrix 
        new_phi = current_phi
        new_y_dot = current_y_dot - np.sin(current_phi) * self.dt
        new_y = current_y + current_y_dot * dt
    ...
    def g_prime(self):
        ...
        print('phi',current_phi)
        # TODO: return the derivative of the g matrix 
        g_prime = np.array([
            [0                             ,0      ,0],
            [-np.cos(current_phi) * self.dt,1      ,0],
            [0                             ,self.dt,1]
        ])
        print('g`',g_prime)
        return g_prime
...
from StateSpaceDisplayFor2D_ import state_space_display_predict
