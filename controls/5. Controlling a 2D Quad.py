...
class Drone2D:
    ...
    def advance_state(self, dt): 
        ...
        # integrate z_dot_dot to get Z_dot
        a_z = self.z_dot_dot
        delta_z_dot = a_z * dt
        self.X[3] += delta_z_dot

        # integrate y_dot_dot to get Y_dot
        a_y = self.y_dot_dot
        delta_y_dot = a_y * dt
        self.X[4] += delta_y_dot

        # integrate phi_dot_dot to get Phi_dot
        a_phi = self.phi_dot_dot
        delta_phi_dot = a_phi * dt
        self.X[5] += delta_phi_dot

        # integrate Z_dot,y_dot,ph_dot to get X position
        delta_X = np.array([self.X[3], self.X[4],self.X[5],0.0,0.0,0.0]) * dt
        self.X += delta_X
    ...
    def set_rotors_angular_velocities(self,linear_acc):
        ...
        # TODO
        self.omega_1 = -1.0 * np.sqrt(0.5 * (self.m / self.k_f) * (self.g - linear_acc))
